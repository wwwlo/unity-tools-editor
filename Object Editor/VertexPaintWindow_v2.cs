...
/*
 * СТРУКТУРА ФАЙЛА VertexPaintWindow_v2.cs:
 * 
 * 1. ОСНОВНОЙ КЛАСС: VertexPaintWindow_v2 : EditorWindow
 ...
 *    └── ColorPaletteData - сериализация палитры цветов
 * 
...
public class VertexPaintWindow_v2 : EditorWindow
{
    [MenuItem("ObjectEditor/Vertex Paint_v2")]
    static void Open() => GetWindow<VertexPaintWindow_v2>("VP");
...
// --------------------------------------------------------
	...
	void SaveMeshToFolder()
	{
		...
		// 1. Проверяем, есть ли уже суффикс __painted в названии меша
		string originalMeshName = workingMesh.name;
		string baseMeshName = originalMeshName;
		
		// Если название уже содержит __painted, не добавляем суффикс повторно
		if (!originalMeshName.EndsWith("__painted"))
		{
			baseMeshName = originalMeshName + "__painted";
		}
    ...
    void LoadPalette()
    {
        ...
        if (!string.IsNullOrEmpty(path))
        {
            try
            {
                ...
                ColorPaletteData paletteData = JsonUtility.FromJson<ColorPaletteData>(json);
    ...
    void SetupTarget(GameObject go)
    {
        ...
        Debug.Log($"Working with mesh: {workingMesh.name}");
        Debug.Log($"Mesh vertex count: {workingMesh.vertexCount}");
        Debug.Log($"Mesh path: {AssetDatabase.GetAssetPath(workingMesh)}");
